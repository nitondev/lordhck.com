...
if [ ! -d "$IMGS_DIR" ]; then
    echo "[i] no $IMGS_DIR directory, skipping."
    exit 0
fi
...
if [ ${#images[@]} -eq 0 ]; then
    echo "[i] no images found in $IMGS_DIR, skipping."
    exit 0
fi

echo "[i] found ${#images[@]} image(s) to process..."
...
echo "[i] done: $converted converted, $skipped skipped."
